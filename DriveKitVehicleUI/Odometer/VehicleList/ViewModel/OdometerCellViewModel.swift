...
struct OdometerCellViewModel {
    ...
    private func getAnalyzedDistanceDescription(localizedDescription: String) -> String {
        return String(format: localizedDescription, self.odometer?.yearAnalyzedDistance.formatKilometerDistance(minDistanceToRemoveFractions: 0, forcedUnitSystem: .international) ?? "0")
    }
    ...
    private func getOdometerDistance() -> String {
        return self.odometer?.distance.formatKilometerDistance(minDistanceToRemoveFractions: 0, forcedUnitSystem: .international) ?? "0"
    }

    private func getAnalyzedDistance() -> String {
        return self.odometer?.analyzedDistance.formatKilometerDistance(minDistanceToRemoveFractions: 0, forcedUnitSystem: .international) ?? "0"
    }

    private func getEstimatedDistance() -> String {
        return self.odometer?.estimatedYearDistance.formatKilometerDistance(minDistanceToRemoveFractions: 0, forcedUnitSystem: .international) ?? "0"
    }
}
